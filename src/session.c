...
tp_stream_engine_session_dispose (GObject *object)
{
  ...
  if (priv->session_proxy)
    {
      g_debug ("%s: disconnecting signals from session proxy", G_STRFUNC);
...
static void
tp_stream_engine_session_finalize (GObject *object)
{
  G_OBJECT_CLASS (tp_stream_engine_session_parent_class)->finalize (object);
}
...
tp_stream_engine_session_class_init (TpStreamEngineSessionClass *klass)
{
  ...
  object_class->finalize = tp_stream_engine_session_finalize;
...
static void
check_start_stream (TpStreamEngineSessionPrivate *priv)
{
...
void
stop_stream (TpStreamEngineSession *self)
{
  TpStreamEngineSessionPrivate *priv = SESSION_PRIVATE (self);
...
static void
prepare_transports (TpStreamEngineSession *self)
{
  TpStreamEngineSessionPrivate *priv = SESSION_PRIVATE (self);
  GPtrArray *codecs;
...
static void
set_stun (TpStreamEngineSession *self)
{
  TpStreamEngineSessionPrivate *priv = SESSION_PRIVATE (self);
                          ...
                          gpointer user_data)
{
  ...
  g_object_get (priv->session_proxy, "name", &bus_name, NULL);
  ...
  const gchar *type)
{
  ...
  g_signal_connect (G_OBJECT (priv->fs_session), "error",
                    G_CALLBACK (cb_fs_session_error), priv->session_proxy);
